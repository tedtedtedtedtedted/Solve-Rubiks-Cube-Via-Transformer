...
size = 4
action_space = ["l", "r", "d", "u", "END"]
action_space_strict = ["l", "r", "d", "u", "END"]
action_dict = {
    "l": (0, 1),
    "r": (0, -1),
    "u": (1, 0),
...
colors = ["Y", "W", "O", "R", "G", "B"]
states = [color1 + color2 +
          color3 for color1 in colors for color2 in colors for color3 in colors]
...
def internal_cube_permute(starting_state: str, moves: str):
    ...
    for i in range(size):
        if 0 in starting_state[i]:
            empty_r = i
            empty_c = starting_state.index(0)
            break
...
def challenge_generator(n: int, repr_mode: str, random_start: bool):
    ...
    if random_start:
        actions_permute_init_state = random.choices(actions, k=20*n*n)
    ...
    if repr_mode == "internal_repr":
        for i in range(n+1):
            state = record[2*i]
            record[2*i] = " ".join([cell for row in state for cell in row])
