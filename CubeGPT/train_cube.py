...
# -----------------------------------------------------------------------------
# Ted: Below overrides above global parameters.
config_keys = [k for k,v in globals().items() if not k.startswith('_') and isinstance(v, (int, float, bool, str))]
exec(open('configurator.py').read()) # overrides from command line or config file # Ted: <exec> essentially inherits all variables in the read external file, just like we declared them here. 
...
scaler = torch.cuda.amp.GradScaler(enabled=(dtype == 'float16')) # Ted: To prevent numerical instability.
...
while True:
    ...
    if iter_num % eval_interval == 0:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                torch.save(checkpoint, os.path.join(out_dir, 'ckpt.pt')) # Ted: ".pt" file is PyTorch's serialized file of a model object. 
    ...
    dt = t1 - t0
    t0 = t1
    if iter_num % log_interval == 0: # Ted: <log_interval> is e.g. print curr iteration report to command line.
