...
from train_15_puzzle import train_from_scratch
from fifteen_puzzle_utilities import challenge_generator, init_state, puzzle_generate_training_file
...
class Agent:
    ...
    def train(self):
        ...
        )  # No need to pass config parameter since hydra.main will automatically fill it in. TODO: Is simply return model sufficient?

    def generate_training_file(self, num_examples: int, num_permute: int):
        puzzle_generate_training_file(num_examples, num_permute)
...
#        return
    ...
    def solve(self, puzzle_file: str, inference_method: str):
        ...
        out_dir = 'out-puzzle_structure'  # ignored if init_from is not 'resume'
        seed = 1337
        ...
        x = (torch.tensor(puzzle_ids, dtype=torch.long,
             device=device)[None, ...])
        # print(x) # DEBUG.
        ...
        with torch.no_grad():
            with ctx:
                y = model.generate(x, inference_method,
...
if __name__ == "__main__":
    agent = Agent()
    # "token" or "action" for <inference_method>.
    agent.solve("PUZZLE_1.txt", "action")
