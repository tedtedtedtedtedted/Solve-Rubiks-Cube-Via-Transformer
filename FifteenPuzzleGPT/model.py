...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not self.flash:
            ...
            extended_mask = torch.tril(torch.ones(
                config.block_size + size_puzzle_tokens, config.block_size))
            mask = torch.narrow(
                extended_mask, 0, size_puzzle_tokens, config.block_size)
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            # Ted: Why does NanoGPT set <attn_mask = None>? Because we use flash attention during inference time not training time.
            y = torch.nn.functional.scaled_dot_product_attention(
                q, k, v, attn_mask=None, dropout_p=self.dropout, is_causal=True)
        else:
            # manual implementation of attention
            # Ted: Dim: [batch_size, num_heads, seq_len, seq_leln].
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        device = idx.device
        # Ted: In "train_cube.py" <model(X, Y)>, <X> is <idx> here so training batch.
        b, t = idx.size()
        ...
        # token embeddings of shape (b, t, n_embd) # Ted: Before embedding [batch_size, seq_len]
        tok_emb = self.transformer.wte(idx)
        # position embeddings of shape (1, t, n_embd)
        pos_emb = self.transformer.wpe(pos)
        # Ted: Pytorch broadcast addition.
        ...
        if targets is not None:
            ...
            # note: using list [-1] to preserve the time dim # Ted: select last token of sequence.
    ...
    @torch.no_grad()
    # TODO: Add max_new_tokens to "config.yaml" and maybe allow larger number like 100 or 200.
    def generate(self, idx, max_new_tokens=35, temperature=1.0, top_k=None):
        """
        ...
        # Ted: Assume input (token) length is at least 1 + 26 + 1.
        assert idx.size(dim=1) >= 1 + 26 + 1
        curr_state = idx[0].tolist()
        # Omit last token which is end-state-separator.
        curr_state = curr_state[(-1 - 26):-1]
        # Ted: Revert back to internal representation from tokens. 1. Need mapping 2. Need input.
        curr_state = [itos[i] for i in curr_state]
        # Ted: Need in color representation not internal representation, and in string format!
        curr_state = internal_to_color(curr_state)
        ...
        for _ in range(max_new_tokens):
            ...
            # Ted: Return a tensor of one number which is the position index <i> and occur with probability <probs[i]>.
            idx_next = torch.multinomial(probs, num_samples=1)

            # append sampled index to the running sequence and continue
            # idx = torch.cat((idx, idx_next), dim=1) # Ted: Found it! Autoregressive with transition function here!
            # Ted: <int()> cast is unnecessary but avoid complaining.
            move = itos[int(idx_next.item())]
            if (move not in action_space):
                ...
                # replace with random move.
                move = random.choice(action_space_strict)
            # Concatenate action.
            # Update <idx_next> (i.e. move).
            idx_next = torch.tensor([[stoi[move]]])
            ...
            separator_state_begin = torch.tensor(
                [[stoi[separator_state_begin]]])
            separator_state_end = torch.tensor([[stoi[separator_state_end]]])
            # Concatenate new state-begin separator.
            idx = torch.cat((idx, separator_state_begin), dim=1)
            # Ted: Note <cube_permute> takes in color representation, not internal representation.
            curr_state = cube_permute(curr_state, move)
            state_tensor = color_to_internal(curr_state)
            # Ted: Encode.
            state_tensor = torch.tensor([[stoi[c] for c in state_tensor]])
            idx = torch.cat((idx, state_tensor), dim=1)
            # Concatenate new state-end separator.
            idx = torch.cat((idx, separator_state_end), dim=1)
